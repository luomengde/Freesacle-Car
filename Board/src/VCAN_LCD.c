 ...
 * Input Param  : size          显示图像大小
 * Input Param  : img           图像地址
 * Output Param : 无
 * Description  : 显示摄像头采集到的图像
 * Author Data  : 2014/6/11 星期三, by Liu
 ******************************************************************************/
 ...
 * Input Param  : 图像位置，显示图像大小，*img，图像大小
 * Output Param : 无
 * Description  : 显示RGB565图像
 * Author Data  : 2015/1/5 星期一  不知道作者
 ******************************************************************************/
void LCD_Img_RGB565_Z(Site_t site,Size_t size,const uint16 *img,Size_t imgsize)
{
    ...
    for( y=0; y < (size.H) ; y++  )	 //在开窗区域的纵轴扫描
    {
      ...
      for( x=0; x< size.W ; x++  )
      {
        ...
        temp = tempY + X;
        LCD_WR_DATA( img[temp] );	 //写图像数据
      }
    }
}
//在LCD上显示浮点数     //注:这个函数的底层还没看，等到有时间了，好好看一看，别太相信别人的函数
