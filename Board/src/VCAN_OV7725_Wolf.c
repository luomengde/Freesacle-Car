...
volatile IMG_STATUS_e      ov7725_wolf_img_flag = IMG_FINISH;   //图像状态
...
void ov7725_wolf_vsync(void)
{
    ...
    //场中断需要判断是场结束还是场开始
    if(ov7725_wolf_img_flag == IMG_START)                   //需要开始采集图像
    {
        ov7725_wolf_img_flag = IMG_GATHER;                  //标记图像采集中
        disable_irq(PORTA_IRQn);

        DMA_EN(CAMERA_DMA_CH);                  //使能通道CHn 硬件请求
        DMA_DADDR(CAMERA_DMA_CH) = (uint32)ov7725_wolf_img_buff;    //恢复地址
    }
    else                                        //图像采集错误
    {
        disable_irq(PORTA_IRQn);                        //关闭PTA的中断
        ov7725_wolf_img_flag = IMG_FAIL;                    //标记图像采集失败
    }
}
...
{
    ov7725_wolf_img_flag = IMG_START;                    //开始采集图像
    PORTA_ISFR = ~0;                                    //写1清中断标志位(必须的，不然回导致一开中断就马上触发中断)
    enable_irq(PORTA_IRQn);                             //允许PTA的中断
    while(ov7725_wolf_img_flag != IMG_FINISH)            //等待图像采集完毕
    {
        if(ov7725_wolf_img_flag == IMG_FAIL)             //假如图像采集错误，则重新开始采集
        {
            ov7725_wolf_img_flag = IMG_START;            //开始采集图像
            PORTA_ISFR = ~0;                            //写1清中断标志位(必须的，不然回导致一开中断就马上触发中断)
            enable_irq(PORTA_IRQn);                     //允许PTA的中断
...
uint8 ov7725_wolf_cfgnum = ARR_SIZE( ov7725_wolf_reg ) ; /*结构体数组成员数目*/
